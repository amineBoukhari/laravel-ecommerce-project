...
class AdminController extends Controller
{
    ...
    public function AddCoupon(Request $request)
    {
        ...
        // Convert inputs to numeric values before validation
        $request->merge([
            'discount_percentage' => is_numeric($request->discount_percentage) ? (float) $request->discount_percentage : 0,
            'min_order_amount' => is_numeric($request->min_order_amount) ? (float) $request->min_order_amount : 0,
        ]);

        // Create Coupon
        Coupon::create([
            'code' => $request->coupon_code,
            'discount_percentage' => (float) $request->discount_percentage, // Ensure numeric
            'min_order_amount' => (float) $request->min_order_amount, // Ensure numeric
        ]);
